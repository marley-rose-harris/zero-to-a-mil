...
export default async (req: Request, context: Context) => {
  ...
  // dedupe — each comment only counts once, ever
  const already = await db.sql`SELECT id FROM processed_comments WHERE comment_id = ${commentId}`;
  if (already.length > 0) {
  ...
  await db.sql`UPDATE participants SET points = points + ${COMMENT_POINTS} WHERE id = ${participantId}`;
  await db.sql`INSERT INTO processed_comments (comment_id, participant_id) VALUES (${commentId}, ${participantId})`;
  await recordStreakActivity(db, participantId);

  return new Response(JSON.stringify({ ok: true, pointsAdded: COMMENT_POINTS }), {
    headers: { "content-type": "application/json" },
  });
};

export const config: Config = {
  path: "/api/webhook/comment",
};
